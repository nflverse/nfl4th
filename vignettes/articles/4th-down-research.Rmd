...
```
# General `nfl4th` recommendations

Let's start by seeing what the model tends to recommend on 4th downs.
...
plot <- pbp %>%
  ...
  select(binned_yardline, yardline_100, ydstogo, go_boost, decision, vegas_wp, score_differential, qtr)
```
...
Here are the worst choices of not going for it (and instead punting or kicking a field goal) in terms of total expected win probability lost.

2020's champion is Kliff Kingsbury.

```{r worst, echo = FALSE, warning = FALSE}
pbp %>%
  filter(
    season == 2020,
    go == 0,
...
Here's a plot that shows that teams' likelihood of going for it is determined in part by how obvious of a decision it is.
```{r league, echo = FALSE, message = FALSE, results = 'hide', fig.keep = 'all', dpi = 600}
...
# Numbers from 2014 - 2020

## Making timelines of teams over time
...
```{r wp_playoff, echo = FALSE, message = FALSE}
current <- pbp %>%
...
## The league is becoming better at 4th downs over time
```{r time, echo = FALSE, message = FALSE, results = 'hide', fig.keep = 'all', dpi = 600}
# labels on the plot
text_df <- tibble(
  label = c(
