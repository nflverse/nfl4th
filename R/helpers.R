...
prepare_df <- function(df) {
  df |>
    ...
    dplyr::select(-dplyr::any_of(drop.cols)) |>
    left_join(
      .games_nfl4th(),
      by = c("home_team", "away_team", "type", "season")
    ) |>
    mutate(
      ...
      home_timeouts_remaining = if_else(
        posteam == home_team,
        posteam_timeouts_remaining,
        defteam_timeouts_remaining
      ),
      away_timeouts_remaining = if_else(
        posteam == away_team,
        posteam_timeouts_remaining,
        defteam_timeouts_remaining
      ),
...
flip_half <- function(df) {
  df |>
    mutate(
      ...
      end_of_half = ifelse(
        qtr == 2 & half_seconds_remaining == 0,
        1,
        0
      ),
      posteam = case_when(
      ...
      score_differential = ifelse(
        posteam != prior_posteam & end_of_half == 1,
        -score_differential,
        score_differential
      ),
